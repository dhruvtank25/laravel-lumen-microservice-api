...
class ProcessingOrderController extends Controller
{
     ...
     */
    public function processingOrdeList(Request $request)
    {
        try {
            $input = $request->all();
            ...
            if ($credentials) {
                ...
                $processing_order_list = Orders::getProcessingOrderList($input['vendor_id']);
                return ResponseBuilder::responseResult($this->successStatus, 'processing Order List has been fetched successfully',$processing_order_list);
            } else {
